const DEPOSIT_ENUMS = {
    deposit_transactions_type: ["Cash In", "Cash Out", "Percent"],
    deposit_line_type: ["Безотзывной"],
}

const DEPOSIT_REGEX = {
    iban: /^BY\d{2}ZEPT\d{4}[a-zA-Z0-9]{16}$/,
    money: /^\d{1,3}(\.\d{3})*,\d{2}$/,
    passport_number: /^[A-Z]{2}\d{7}$/,
    passport_id_number: /^[A-Z0-9]{14}$/,
    full_name: /^[а-яА-ЯёЁ]+$/,
    deposit_contract_number: /^[a-zA-Z0-9]{6}[0-9]{10}$/,
    ...
    second_order_synthetic_bank_precent_account_number: /^\d{15}$/
}
